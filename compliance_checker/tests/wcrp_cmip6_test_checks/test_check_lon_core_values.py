...
from compliance_checker.checks.variable_checks.check_lon_core_values import check_lon_core_values
...
def test_lon_core_values_range_and_monotonic(tmp_path):
    ...
    with Dataset(p, "w") as ds:
        ...
        lon[:] = [0.0, 350.0, 349.0, 361.0]  # not non-decreasing and out of [0,360]
    with Dataset(p, "r") as ds:
        res = check_lon_core_values(ds, "lon", severity=BaseCheck.MEDIUM)
    msgs = "\n".join(m for r in res for m in r.msgs)
    assert "V072" in msgs   # monotonic
    assert "V074" in msgs   # range

...
def check_lon_within_bounds(ds, coord_name="lon", severity=BaseCheck.MEDIUM, tol=1e-12):
    ...
    try:
        ...
        ctx.add_failure(f"V075: Error reading longitude values: {e}")
        return [ctx.to_result()]
    ...
    if bvar is not None:
        try:
            ...
            # ill-formed or unreadable bounds -> treat as no-op for this check
            bnds = None

    check_within_bounds(ctx, arr, bnds, id_within="V075", tol=tol)
    if not ctx.msgs:
        ctx.add_pass()
    return [ctx.to_result()]
